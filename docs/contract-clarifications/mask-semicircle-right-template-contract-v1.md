...
Status: FROZEN for Phase C6 · Canonical document 1.7.0

## 문제

`MASK_SEMICIRCLE_RIGHT.png`는 원형 이미지와 상단 logo cutout을 함께 보여주지만, 기존
단일 이미지 capability에는 logo asset, mask digest, safe box, white validation을
...
- 실행 slot은 `IMAGE_PRIMARY` 다음 `LOGO_PRIMARY` 두 개로 고정한다.
...
- logo는 PNG/alpha/transparent/visible RGB >= 240, ALPHA_TRIM_CONTAIN, CENTER,
  safe box `(847,24,126,44)`, max upscale 1.5×를 사용한다.
- missing/invalid logo 또는 mask는 전체 artifact를 BLOCKED로 만들며 publish/download를
  허용하지 않는다.
- 기존 `1.1.0` input plans는 읽기 호환하고 공개 Integration Contract/output capability는
  `1.2.0`으로 동결한다.
...
Canonical 문서는 1.6.2→1.7.0, Template Contract는 1.3.0→1.4.0,
Desktop은 0.5.2→0.6.0, Integration은 1.1.0→1.2.0으로 증가한다. 기존 단일-slot
