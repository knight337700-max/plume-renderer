...
export function FreeformEditor() {
  ...
  const previewFresh = Boolean(preview && preview.requestSequence === requestSequence && preview.validationStatus !== "ERROR" && preview.errors.length === 0);
  ...
  async function requestPreview(): Promise<void> {
    ...
    try {
      ...
      setPreview(result);
      if (result.validationStatus === "ERROR") setNotice("Core Validator ERROR · Export가 차단되었습니다.");
