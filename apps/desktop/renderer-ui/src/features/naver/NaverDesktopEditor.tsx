    ...
    <section className="naver-lab" data-testid="naver-editor" data-primary-selected={primary ? "true" : "false"}>
      <aside className="naver-sidebar" aria-label="NAVER editor">
        ...
        {placementId === "NAVER_SMARTCHANNEL" ? <section className="naver-card" data-testid="naver-smartchannel-editor">
          ...
          <div className="font-preflight" data-testid="naver-smartchannel-font-preflight"><strong>SmartChannel official font preflight</strong><span>{catalog.fontPreflight.configuredDirectory || "공식 NanumBarunGothic/Sandoll runtime asset 필요"}</span>{catalog.fontPreflight.requiredAssets.map((font) => <small key={font.token}>{font.token} · {font.expectedFilename} · {font.expectedSha256 || "SHA unresolved"}</small>)}<small>공식 자산이 없으면 SmartChannel render/download만 차단됩니다.</small></div>
        </section> : null}
