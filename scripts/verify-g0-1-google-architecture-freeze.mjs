...
check("runtime_google_profiles_absent", (g1Implemented || g2Implemented) ? activeGoogleHits.every((relativePath) => relativePath.startsWith("src/core/google-static") || relativePath === "src/core/index.ts" || relativePath.startsWith("packages/renderer-contract/src/google-static") || relativePath === "packages/renderer-contract/src/index.ts" || relativePath.startsWith("packages/renderer-contract/dist/")) : activeGoogleHits.length === 0, (g1Implemented || g2Implemented) ? `${activeGoogleHits.length} expected Google implementation file(s)` : (activeGoogleHits.join(",") || "no Google runtime profile/implementation/golden text"));
