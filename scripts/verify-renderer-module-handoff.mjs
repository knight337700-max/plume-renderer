...
if (!(await exists(root))) {
  ...
  const required = ["README.md", "MANIFEST.json", "contracts", "src", "packages", "scripts", "tests", "fixtures", "reference", "docs", "source-guides", "local-runtime-resources", "package.json", "pnpm-lock.yaml"];
  ...
  check("naver_bundled_font_manifest", naverFontManifest?.status === "RESOLVED_BUNDLED_PSD_EXACT" && naverFontManifest?.files?.filter((entry) => entry.bundled === true && entry.runtime === true && entry.fallback === false).length === 3, JSON.stringify({ status: naverFontManifest?.status, files: naverFontManifest?.files?.length, runtimeFiles: naverFontManifest?.files?.filter((entry) => entry.runtime === true).length }));
  ...
  check("n7_runtime_manifest", manifest?.handoffPhase === "N7_7_SMARTCHANNEL_PSD_EXACT_RUNTIME_FONT_CORRECTION" && manifest?.versions?.rendererCore === "0.8.4" && manifest?.versions?.desktop === "0.9.7", JSON.stringify({ phase: manifest?.handoffPhase, rendererCore: manifest?.versions?.rendererCore, desktop: manifest?.versions?.desktop }));
  ...
  check("n7_7_correction", manifest?.handoffPhase === "N7_7_SMARTCHANNEL_PSD_EXACT_RUNTIME_FONT_CORRECTION" && correctionAudit?.phase?.status === "PASS" && correctionAudit?.acceptanceEvidence?.templatesPassed === 120 && correctionAudit?.acceptanceEvidence?.providerParity?.status === "PASS", JSON.stringify({ phase: manifest?.handoffPhase, status: correctionAudit?.phase?.status, templates: correctionAudit?.acceptanceEvidence?.templatesPassed, parity: correctionAudit?.acceptanceEvidence?.providerParity?.status }));
