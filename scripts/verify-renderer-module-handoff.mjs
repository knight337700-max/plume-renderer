...
if (!(await exists(root))) {
  ...
  const required = ["README.md", "MANIFEST.json", "artifacts/n7-7-4", "contracts", "src", "packages", "scripts", "tests", "fixtures", "reference", "docs", "source-guides", "local-runtime-resources", "package.json", "pnpm-lock.yaml"];
  ...
  check("n7_runtime_manifest", manifest?.handoffPhase === "N7_7_4_MACOS_ORIGINAL_TTC_RENDERER_INTEGRATION" && manifest?.versions?.rendererCore === "0.8.5" && manifest?.versions?.desktop === "0.9.8", JSON.stringify({ phase: manifest?.handoffPhase, rendererCore: manifest?.versions?.rendererCore, desktop: manifest?.versions?.desktop }));
