...
if (!(await exists(root))) {
  ...
  const isG1Handoff = sourceHandoffPhase === "G1_GOOGLE_STATIC_CONTRACTS_AND_PROFILE_IMPLEMENTATION";
  if (["G0_GOOGLE_ADS_STATIC_CAPABILITY_DISCOVERY_AND_ARCHITECTURE", "G0_1_GOOGLE_ARCHITECTURE_ACCEPTANCE_AND_FREEZE", "G1_GOOGLE_STATIC_CONTRACTS_AND_PROFILE_IMPLEMENTATION"].includes(sourceHandoffPhase) && manifest) manifest.handoffPhase = "M2_3_META_USER_VISUAL_ACCEPTANCE_AND_GOLDEN_FREEZE";
  const required = ["README.md", "MANIFEST.json", "artifacts/n7-7-4", "artifacts/n7-7-5", "artifacts/n7-7-6", "artifacts/n7-8", "artifacts/n8", "artifacts/m0", "artifacts/m1", "artifacts/m2", "artifacts/m2-1", "artifacts/m2-2", "artifacts/m2-2a", "artifacts/m2-3", "artifacts/g0", "artifacts/g0-1", "artifacts/g1", "contracts", "contracts/goldens/meta-static-goldens.json", "contracts/google", "contracts/google/architecture.g0.json", "contracts/google/architecture-freeze.g0.1.json", "contracts/google/static-asset-profiles.g1.json", "contracts/google/capability-asset-role-mapping.g1.json", "contracts/google/target-constraints.g1.json", "contracts/google/creative-asset-set-manifest.schema.json", "contracts/google/delivery-set-validator.g1.json", "contracts/google/diagnostics.g1.json", "artifacts/g0-1/google-static-architecture-freeze-verification.json", "artifacts/g1/google-static-contracts-profile-verification.json", "src", "packages", "scripts", "tests", "fixtures", "reference", "docs", "source-guides", "local-runtime-resources", "package.json", "pnpm-lock.yaml"];
  ...
  const g0_1GooglePhase = "G0_1_GOOGLE_ARCHITECTURE_ACCEPTANCE_AND_FREEZE";
  const g0_1GoogleFreezeRegistry = await readJson("contracts/google/architecture-freeze.g0.1.json");
  const g0_1GoogleEvidence = await readJson("artifacts/g0-1/google-static-architecture-freeze-verification.json");
  const g1GoogleProfiles = await readJson("contracts/google/static-asset-profiles.g1.json");
  const g1GoogleMapping = await readJson("contracts/google/capability-asset-role-mapping.g1.json");
  const g1GoogleConstraints = await readJson("contracts/google/target-constraints.g1.json");
  const g1GoogleDiagnostics = await readJson("contracts/google/diagnostics.g1.json");
  const g1GoogleEvidence = await readJson("artifacts/g1/google-static-contracts-profile-verification.json");
  const g1GoogleVersion = await readJson("contracts/contract-versions.json");
  const g0GoogleImplementation = g0GoogleArchitecture?.implementation ?? {};
  check("g0_google_architecture", [g0GooglePhase, g0_1GooglePhase, "G1_GOOGLE_STATIC_CONTRACTS_AND_PROFILE_IMPLEMENTATION"].includes(sourceHandoffPhase) && manifest?.g0GoogleStaticDiscovery?.status === "PASS" && manifest?.g0GoogleStaticDiscovery?.architectureStatus === "FREEZE_CANDIDATE" && manifest?.g0GoogleStaticDiscovery?.repositoryApplication === "APPLIED_ARCHITECTURE_ONLY" && g0GoogleArchitecture?.phase === g0GooglePhase && g0GoogleArchitecture?.status === "FREEZE_CANDIDATE" && g0GoogleArchitecture?.repositoryApplication === "APPLIED_ARCHITECTURE_ONLY" && g0GoogleCapabilities?.runtimeEnabled === false && g0GoogleEvidence?.status === "PASS", JSON.stringify({ sourceHandoffPhase, handoff: manifest?.g0GoogleStaticDiscovery, architecture: g0GoogleArchitecture?.status, evidence: g0GoogleEvidence?.status }));
  check("g0_google_capability_boundary", g0GoogleCapabilities?.capabilities?.length === 7 && g0GoogleArchitecture?.compositionBoundary?.singleArtifact === true && g0GoogleArchitecture?.compositionBoundary?.deliveryCollectionSeparate === true && g0GoogleArchitecture?.compositionBoundary?.displayAndPmaxProfilesMerged === false && ["0.1.0", "1.0.0"].includes(manifest?.versions?.googleStatic?.architectureVersion) && manifest?.versions?.googleStatic?.status === "PASS", JSON.stringify({ capabilities: g0GoogleCapabilities?.capabilities?.length, boundary: g0GoogleArchitecture?.compositionBoundary, versions: manifest?.versions?.googleStatic }));
  ...
  check("g0_1_google_architecture_freeze", [g0_1GooglePhase, "G1_GOOGLE_STATIC_CONTRACTS_AND_PROFILE_IMPLEMENTATION"].includes(sourceHandoffPhase) && manifest?.g0_1GoogleArchitectureFreeze?.status === "PASS" && manifest?.g0_1GoogleArchitectureFreeze?.architectureStatus === "FROZEN" && manifest?.g0_1GoogleArchitectureFreeze?.architectureVersion === "1.0.0" && g0_1GoogleFreezeRegistry?.status === "FROZEN" && g0_1GoogleFreezeRegistry?.googleArchitectureVersion === "1.0.0" && g0_1GoogleEvidence?.status === "PASS" && g0_1GoogleEvidence?.architectureStatus === "FROZEN", JSON.stringify({ sourceHandoffPhase, handoff: manifest?.g0_1GoogleArchitectureFreeze, registry: g0_1GoogleFreezeRegistry?.status, evidence: g0_1GoogleEvidence?.status }));
  check("g0_1_google_counts_and_boundary", g0_1GoogleFreezeRegistry?.authoritativeRecords?.length === 6 && g0_1GoogleFreezeRegistry?.counts?.capabilities === 7 && g0_1GoogleFreezeRegistry?.counts?.demandGenUploadedPresets === 7 && g0_1GoogleFreezeRegistry?.counts?.legacyDisplayCanvases === 20 && g0_1GoogleFreezeRegistry?.counts?.unresolvedRules === 9 && g0_1GoogleFreezeRegistry?.counts?.diagnostics === 11 && g0_1GoogleFreezeRegistry?.invariants?.frozenOutputChanges === 0 && g0_1GoogleFreezeRegistry?.invariants?.runtimeNetworkAccess === "PROHIBITED" && g0_1GoogleFreezeRegistry?.invariants?.unresolvedRuleStatus === "UNRESOLVED_FAIL_CLOSED", JSON.stringify({ records: g0_1GoogleFreezeRegistry?.authoritativeRecords?.length, counts: g0_1GoogleFreezeRegistry?.counts, invariants: g0_1GoogleFreezeRegistry?.invariants }));
  check("g0_1_google_runtime_absent", manifest?.g0_1GoogleArchitectureFreeze?.runtimeProfilesAdded === false && manifest?.g0_1GoogleArchitectureFreeze?.rendererCodeAdded === false && manifest?.g0_1GoogleArchitectureFreeze?.validatorRuntimeAdded === false && manifest?.g0_1GoogleArchitectureFreeze?.goldensAdded === false && manifest?.g0_1GoogleArchitectureFreeze?.desktopUiAdded === false && manifest?.g0_1GoogleArchitectureFreeze?.uploadIntegrationAdded === false && manifest?.g0_1GoogleArchitectureFreeze?.runtimeNetworkAccess === "PROHIBITED" && Array.isArray(manifest?.g0_1GoogleArchitectureFreeze?.plumeDependencies) && manifest?.g0_1GoogleArchitectureFreeze?.plumeDependencies.length === 0 && manifest?.g0_1GoogleArchitectureFreeze?.frozenChannelOutputChanges === 0 && g0_1GoogleFreezeRegistry?.g1GateStatus === "OPEN", JSON.stringify(manifest?.g0_1GoogleArchitectureFreeze));
  check("g0_1_google_provenance", manifest?.sourceProvenance?.g0_1GoogleFreezeRegistry === "contracts/google/architecture-freeze.g0.1.json" && manifest?.sourceProvenance?.g0_1GoogleEvidenceDirectory === "artifacts/g0-1" && manifest?.sourceProvenance?.g0_1GoogleVerification === "artifacts/g0-1/google-static-architecture-freeze-verification.json" && manifest?.sourceProvenance?.g0_1GoogleImplementationRecord === "docs/implementation/google-ads-static-architecture-freeze-g0-1.md" && manifest?.sourceProvenance?.g0_1GoogleAdr === "docs/adr/ADR-0059-google-static-architecture-freeze-g0-1.md" && manifest?.sourceProvenance?.g0_1GoogleVerifier === "scripts/verify-g0-1-google-architecture-freeze.mjs", JSON.stringify(manifest?.sourceProvenance));
  check("g1_google_static_contracts", sourceHandoffPhase === "G1_GOOGLE_STATIC_CONTRACTS_AND_PROFILE_IMPLEMENTATION" && manifest?.g1GoogleStaticContracts?.status === "PASS" && manifest?.g1GoogleStaticContracts?.architectureVersion === "1.0.0" && manifest?.g1GoogleStaticContracts?.contractVersion === "1.0.0" && manifest?.g1GoogleStaticContracts?.profileCount === 14 && manifest?.g1GoogleStaticContracts?.geometryProfileCount === 7 && manifest?.g1GoogleStaticContracts?.uploadedDisplayStaticProfileCount === 7 && manifest?.g1GoogleStaticContracts?.legacyDisplayRuntimeProfiles === 0 && manifest?.g1GoogleStaticContracts?.capabilityCount === 7 && manifest?.g1GoogleStaticContracts?.diagnostics === 11 && manifest?.g1GoogleStaticContracts?.runtimeNetworkAccess === "PROHIBITED" && Array.isArray(manifest?.g1GoogleStaticContracts?.plumeDependencies) && manifest?.g1GoogleStaticContracts?.plumeDependencies.length === 0 && g1GoogleProfiles?.status === "IMPLEMENTED" && g1GoogleProfiles?.profileCount === 14 && g1GoogleMapping?.capabilityCount === 7 && g1GoogleMapping?.compositionBoundary?.legacyRuntimeProfiles === 0 && g1GoogleConstraints?.byteUnit === "decimal-byte" && g1GoogleDiagnostics?.count === 11 && g1GoogleEvidence?.status === "PASS" && g1GoogleVersion?.canonicalPhaseG1Google?.documentCurrent === "1.25.0", JSON.stringify({ sourceHandoffPhase, handoff: manifest?.g1GoogleStaticContracts, profiles: g1GoogleProfiles?.profileCount, diagnostics: g1GoogleDiagnostics?.count, evidence: g1GoogleEvidence?.status }));
