...
check("active_google_runtime_absent", (g1Implemented || g2Implemented) ? activeGoogleHits.every((relativePath) => relativePath.startsWith("src/core/google-static") || relativePath === "src/core/index.ts" || relativePath.startsWith("packages/renderer-contract/src/google-static") || relativePath === "packages/renderer-contract/src/index.ts" || relativePath.startsWith("packages/renderer-contract/dist/")) : activeGoogleHits.length === 0, (g1Implemented || g2Implemented) ? `${activeGoogleHits.length} expected Google implementation file(s)` : (activeGoogleHits.join(",") || "no active Google profile/runtime text"));
...
const frozenDiff = execFileSync("git", ["diff", "--name-only", baseline, "--", "src", "apps", "packages", "contracts/freeform-format-profiles.json", "fixtures/golden"], { cwd: root, encoding: "utf8" }).trim().split(/\r?\n/u).filter((relativePath) => relativePath && !((g1Implemented || g2Implemented) && ["src/core/google-static.ts", "src/core/google-static-render.ts", "src/core/index.ts", "packages/renderer-contract/src/google-static.ts", "packages/renderer-contract/src/index.ts"].includes(relativePath)) && !(g2_1Implemented && relativePath.replaceAll("\\", "/").startsWith("fixtures/golden/google/"))).join("\n");
