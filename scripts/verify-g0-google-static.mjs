...
check("active_google_runtime_absent", g3Implemented ? activeGoogleHits.length > 0 : (g1Implemented || g2Implemented) ? activeGoogleHits.every((relativePath) => relativePath.startsWith("src/core/google-static") || relativePath === "src/core/index.ts" || relativePath.startsWith("packages/renderer-contract/src/google-static") || relativePath === "packages/renderer-contract/src/index.ts" || relativePath.startsWith("packages/renderer-contract/dist/")) : activeGoogleHits.length === 0, g3Implemented ? `${activeGoogleHits.length} additive Google implementation file(s)` : (g1Implemented || g2Implemented) ? `${activeGoogleHits.length} expected Google implementation file(s)` : (activeGoogleHits.join(",") || "no active Google profile/runtime text"));
check("google_runtime_paths_absent", !(await exists("src/core/google")) && !(await exists("apps/desktop/renderer/src/google")) && (g2_1Implemented || !(await exists("fixtures/golden/google"))), "no Google runtime directory; G2.1 frozen Golden directory is additive");
...
check("diagnostics_not_in_active_registry", g3Implemented ? [...googleCodes].every((code) => activeErrorRegistryText.includes(code)) : [...googleCodes].every((code) => !activeErrorRegistryText.includes(code) && !activeIntegrationErrorText.includes(code)), g3Implemented ? "G3 Google diagnostics are active Error Registry entries" : "Google diagnostics are not active Error Registry entries");

const frozenDiff = execFileSync("git", ["diff", "--name-only", baseline, "--", "src", "apps", "packages", "contracts/freeform-format-profiles.json", "fixtures/golden"], { cwd: root, encoding: "utf8" }).trim().split(/\r?\n/u).filter((relativePath) => relativePath && !(g3Implemented && (relativePath.startsWith("apps/desktop/") || relativePath === "src/core/google-static.ts" || relativePath === "src/core/google-static-render.ts" || relativePath === "src/core/index.ts" || relativePath.startsWith("packages/renderer-contract/src/google-static") || relativePath === "packages/renderer-contract/src/index.ts")) && !g3Implemented && !((g1Implemented || g2Implemented) && ["src/core/google-static.ts", "src/core/google-static-render.ts", "src/core/index.ts", "packages/renderer-contract/src/google-static.ts", "packages/renderer-contract/src/index.ts"].includes(relativePath)) && !(g2_1Implemented && relativePath.replaceAll("\\", "/").startsWith("fixtures/golden/google/"))).join("\n");
...
check("canonical_version_unchanged", g3_0_3Implemented ? versions?.canonicalPhaseG3_0_3Google?.documentCurrent === "1.29.0" : g3RevisionImplemented ? versions?.canonicalPhaseG3_0_2Google?.documentCurrent === "1.28.1" : g3Implemented ? versions?.canonicalPhaseG3Google?.documentCurrent === "1.28.0" : canonical.startsWith("# Kakao Bizboard Local Renderer Specification v1") && /Document version:\*\* (?:1\.23\.1|1\.24\.0|1\.25\.0|1\.26\.0|1\.27\.0)/u.test(canonical) && canonical.includes("Phase G0") && versions?.canonicalPhaseG0Google?.documentCurrent === "1.23.1" && versions?.canonicalPhaseG0Google?.documentBump === "none" && versions?.canonicalPhaseG0_1Google?.documentCurrent === "1.24.0" && (!g1Implemented || versions?.canonicalPhaseG1Google?.documentCurrent === "1.25.0") && (!g2Implemented || versions?.canonicalPhaseG2Google?.documentCurrent === "1.26.0") && (!g2_1Implemented || versions?.canonicalPhaseG2_1Google?.documentCurrent === "1.27.0"), JSON.stringify({ historical: versions?.canonicalPhaseG0Google?.documentCurrent, freeze: versions?.canonicalPhaseG0_1Google?.documentCurrent, g1: versions?.canonicalPhaseG1Google?.documentCurrent, g2: versions?.canonicalPhaseG2Google?.documentCurrent, g2_1: versions?.canonicalPhaseG2_1Google?.documentCurrent, g3: versions?.canonicalPhaseG3Google?.documentCurrent, g3Revision: versions?.canonicalPhaseG3_0_2Google?.documentCurrent, g3_0_3: versions?.canonicalPhaseG3_0_3Google?.documentCurrent }));
