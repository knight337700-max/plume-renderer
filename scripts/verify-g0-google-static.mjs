...
const frozenDiff = execFileSync("git", ["diff", "--name-only", baseline, "--", "src", "apps", "packages", "contracts/freeform-format-profiles.json", "fixtures/golden"], { cwd: root, encoding: "utf8" }).trim().split(/\r?\n/u).filter((relativePath) => relativePath && !((g1Implemented || g2Implemented) && ["src/core/google-static.ts", "src/core/google-static-render.ts", "src/core/index.ts", "packages/renderer-contract/src/google-static.ts", "packages/renderer-contract/src/index.ts"].includes(relativePath))).join("\n");
...
check("canonical_version_unchanged", canonical.startsWith("# Kakao Bizboard Local Renderer Specification v1") && /Document version:\*\* (?:1\.23\.1|1\.24\.0|1\.25\.0|1\.26\.0)/u.test(canonical) && canonical.includes("Phase G0") && versions?.canonicalPhaseG0Google?.documentCurrent === "1.23.1" && versions?.canonicalPhaseG0Google?.documentBump === "none" && versions?.canonicalPhaseG0_1Google?.documentCurrent === "1.24.0" && (!g1Implemented || versions?.canonicalPhaseG1Google?.documentCurrent === "1.25.0") && (!g2Implemented || versions?.canonicalPhaseG2Google?.documentCurrent === "1.26.0"), JSON.stringify({ historical: versions?.canonicalPhaseG0Google?.documentCurrent, freeze: versions?.canonicalPhaseG0_1Google?.documentCurrent, g1: versions?.canonicalPhaseG1Google?.documentCurrent, g2: versions?.canonicalPhaseG2Google?.documentCurrent }));
