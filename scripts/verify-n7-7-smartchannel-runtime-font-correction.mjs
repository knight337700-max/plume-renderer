...
check("version_policy", versions?.documentVersion?.current === "1.21.2" && versions?.canonicalPhaseN7_7?.rendererCoreVersion === "0.8.4" && versions?.canonicalPhaseN7_7?.desktopCurrent === "0.9.7" && versions?.templateContractVersion === "1.9.0", JSON.stringify(versions?.canonicalPhaseN7_7));
...
check("typography_correction", typography?.runtimeCorrection?.phase === "N7_7_SMARTCHANNEL_PSD_EXACT_RUNTIME_FONT_CORRECTION" && typography?.runtimeCorrection?.tokenIdsPreserved === true && typography?.runtimeResolution === "PSD_EXACT_RENDERER_OWNED", JSON.stringify(typography?.runtimeCorrection));
...
const fingerprintSource = coreText.slice(coreText.indexOf("const pixelFingerprint"), coreText.indexOf("const renderFingerprint"));
check("core_logical_fingerprint", fingerprintSource.includes("font.token") && fingerprintSource.includes("font.digest") && !fingerprintSource.includes("font.path") && !fingerprintSource.includes("resolvedPath"), fingerprintSource || "pixel fingerprint construction not found");
