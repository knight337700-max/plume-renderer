...
check("m2_3_canonical_document", canonicalDocument.includes("Phase M2.3") && canonicalDocument.includes("META user visual acceptance and Golden freeze") && /\*\*Document version:\*\* (?:1\.23\.1|1\.24\.0)/u.test(canonicalDocument), "canonical document records the M2.3 freeze without changing template coordinates");
