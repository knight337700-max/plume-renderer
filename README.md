...
1. `이미지 선택`에서 제품 이미지(PNG/JPG/JPEG)를 선택한다. OBJECT_RIGHT는 투명 PNG만, THUMBNAIL_BOX_RIGHT·THUMBNAIL_MULTI_RIGHT·MASK_SEMICIRCLE_RIGHT의 IMAGE_PRIMARY는 PNG/JPG/JPEG를 허용한다. Multi는 두 Slot을 각각 선택하거나 Primary Asset을 명시적으로 재사용하고, MASK는 `흰색 로고 PNG 선택`으로 필수 LOGO_PRIMARY를 지정한다.
...
release/Kakao-Bizboard-Local-Renderer-0.6.0-x64.exe
```
...
- MASK circle `(801,225,r=180)`, image destination `(621,45,360,213)`, LOGO_PRIMARY safe box `(847,24,126,44)`, pinned mask digest and white RGB threshold `240`
- CTA `NONE`만 활성
...
`packages/renderer-contract/`와 Integration Contract 문서가 별도 `schemaVersion: 1.2.0` 계약이다. JSON에는 절대 경로 또는 바이너리 객체를 넣지 않고 `assetRef`를 Runtime Asset Resolver로 해석한다. 입력 Asset은 PNG/JPEG만 허용하며 WebP/GIF/AVIF/BMP/TIFF/SVG는 Production Capability에서 차단한다. 현재 production Capability는 `KAKAO_BIZBOARD_OBJECT_RIGHT`, `KAKAO_BIZBOARD_THUMBNAIL_BOX_RIGHT`, `KAKAO_BIZBOARD_THUMBNAIL_MULTI_RIGHT`, `KAKAO_BIZBOARD_MASK_SEMICIRCLE_RIGHT`다. MASK는 ordered `IMAGE_PRIMARY` + required white `LOGO_PRIMARY` 두 Slot, pinned analytic mask, logo alpha/transparent/RGB validation을 사용한다. OBJECT_RIGHT는 `ALPHA_TRIM_CONTAIN + CONTAIN`과 alpha PNG만, Thumbnail Box/Multi는 PNG/JPEG semantic/manual crop을 실제 구현한다. JPEG EXIF Orientation은 crop 전에 보정한다. Candidate와 Subject Protection은 입력 Plan을 검증하고 자동 Crop 생성이나 자동 중심 대체를 하지 않는다.
